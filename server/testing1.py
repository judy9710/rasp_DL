...
def maskdetect(file) : 
    file_path='./VideoImage'+'/'+'Saved'+'/'+file
...
# Showing informations on the screen
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            cv2.putText(img, label, (x, y + 30), font, 1, (126,255,12), 1)
         
            cv2.imwrite(mask_dir+'/'+file,img)
            if label == 'Wearing Mask':
                return 0
            elif label == 'Not Wearing Mask':
                return 1
            cv2.waitKey(0)
